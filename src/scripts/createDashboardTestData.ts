...
async function createDashboardTestData() {
  try {
    ...
    // Create sample blogs if they don't exist
    await pool.query(`
      ...
      VALUES 
        ...
        ('Best Time to Visit Rwanda', 'Learn about the different seasons and when to plan your visit...', 'Planning your trip to Rwanda? Here\'s when you should visit.', 'best-time-to-visit-rwanda', false, 1, NOW() - INTERVAL '30 days', NOW() - INTERVAL '30 days'),
        ('Rwanda Conservation Success Story', 'How Rwanda became a leader in conservation efforts...', 'The inspiring story of Rwanda\'s conservation success.', 'rwanda-conservation-story', true, 1, NOW() - INTERVAL '45 days', NOW() - INTERVAL '45 days')
    ...
    await pool.query(`
      ...
      FROM blogs b
      CROSS JOIN generate_series(1, floor(random() * 50 + 10)::int) g
    ...
    await pool.query(`
      ...
      VALUES 
        ...
        (5, 2, 'Inspiring story about Rwanda\'s conservation efforts. More countries should follow this example.', true, NOW() - INTERVAL '44 days'),
        (1, 3, 'Which safari destination would you recommend for first-time visitors?', true, NOW() - INTERVAL '4 days')
      ON CONFLICT DO NOTHING
    `)
    ...
    await pool.query(`
      ...
      VALUES 
        ...
        ('Sarah', 'Williams', 'sarah.williams@example.com', '+250785432109', false, NOW() - INTERVAL '15 days', NOW()),
        ('David', 'Brown', 'david.brown@example.com', '+250784321098', true, NOW() - INTERVAL '120 days', NOW())
    ...
    await pool.query(`
      ...
      VALUES 
        ...
        (3, 1, 4, 'Well-organized tour with lots of cultural insights. Highly recommend!', true, NOW() - INTERVAL '40 days'),
        (2, 4, 5, 'Perfect vacation! Everything was well planned and executed.', true, NOW() - INTERVAL '25 days')
      ON CONFLICT DO NOTHING
    `)
    ...
    await pool.query(`
      ...
      VALUES 
        ...
        ('David Lee', 'david@example.com', 'Partnership', 'We would like to discuss partnership opportunities.', 'new', NOW() - INTERVAL '1 day')
      ON CONFLICT DO NOTHING
    `)
    ...
    await pool.query(`
      ...
      VALUES 
        ...
        ('info@example.com', true, NOW() - INTERVAL '90 days'),
        ('contact@example.com', false, NOW() - INTERVAL '15 days')
      ON CONFLICT (email) DO NOTHING
    `)
    ...
    await pool.query(`
      ...
      VALUES 
        ...
        (1, 3, 'John Doe', 'john.doe@example.com', '+250788123456', 1800, 'confirmed', NOW() + INTERVAL '20 days', NOW() - INTERVAL '15 days', NOW() - INTERVAL '15 days'),
        (5, 2, 'David Brown', 'david.brown@example.com', '+250784321098', 3200, 'confirmed', NOW() + INTERVAL '90 days', NOW() - INTERVAL '20 days', NOW() - INTERVAL '20 days')
      ON CONFLICT DO NOTHING
    `)
